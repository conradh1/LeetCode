...
public class LongestAbsoluteFilePath {
  ...
  public static int lengthLongestPath(String input) {
    ...
    for (String level: input.split("\n")) {
      int lev = level.lastIndexOf("\t")+1; // number of "\t"

      while (lev+1 < stack.size()) {
	stack.pop();
      }
      int len = stack.peek()+level.length()-lev+1; // remove "/t", add"/"
      stack.push(len);
      if ( level.contains(".")) max = Math.max(max, len-1);
